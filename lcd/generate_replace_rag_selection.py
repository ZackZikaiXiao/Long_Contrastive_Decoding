        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            if calibration_enable:
                ...
                # 计算噪声logits
                model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
                model_inputs["position_ids"][0, 0] = model_inputs["cache_position"][0]

                # topk_vector = torch.load('topk_vector.pt').to(model_inputs["attention_mask"].device).long()
 
                if 'cached_topk_vector' not in locals():
                    cached_topk_vector = torch.load('topk_vector.pt').to(model_inputs["attention_mask"].device).long()

                topk_vector = cached_topk_vector

                # 如果 topk_vector 比 attention_mask 更短，则在后面补1
                if topk_vector.shape[0] < model_inputs["attention_mask"].shape[1]:
                    padding_length = model_inputs["attention_mask"].shape[1] - topk_vector.shape[0]
                    padding = torch.ones(padding_length, device=topk_vector.device, dtype=topk_vector.dtype)
                    topk_vector = torch.cat([topk_vector, padding])

                # 原地修改 attention_mask
                # model_inputs["attention_mask"][0] *= (1 - topk_vector)
                model_inputs["attention_mask"][0].fill_(1)
                model_inputs["attention_mask"][0] *= topk_vector
                # model_inputs["attention_mask"][0][200:290] = 0
                model_inputs["attention_mask"][0][0:100] = 1
                model_inputs["attention_mask"][0][-100:] = 1
                ...
                # next_token_logits = (1 + 1.5) * next_token_logits_base - 1.5 * next_token_logits_noise
                ...
                # next_token_logits = next_token_logits_base
            else:
